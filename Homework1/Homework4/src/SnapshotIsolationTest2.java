 ...
 *
 */
public class SnapshotIsolationTest2 {
				...
				if(step < schedule[i].length && schedule[i][step] != null) {
					...
					try {
						if(xactLabelToXact.containsKey(xactLabel)) {
							...
							xact = SnapshotIsolation.begin_transaction();
							xactLabelToXact.put(xactLabel, xact);
							System.out.println("Creating new transaction " + Integer.toString(xact));
						}
						if(xactOps.length == 1) {
							switch(xactOps[0]) {
								case COMMIT: SnapshotIsolation.commit(xact); break;
								case ABORT: SnapshotIsolation.rollback(xact); break;
							}
						} else {
							switch(xactOps[0]) {
								case WRITE: SnapshotIsolation.write(xact, xactOps[1], getValue(step)); break;
								case READ: {
									int readValue = SnapshotIsolation.read(xact, xactOps[1]);
